...
void __EntryFunction__()
{
	...
	if (ENTITY::DOES_ENTITY_EXIST(uScriptParam_0))
	{
		...
		vLocal_2 = { ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(uScriptParam_0, 0f, 0f, 0f) };
...
int func_2()
{
	...
	if (!PED::IS_PED_INJURED(PLAYER::PLAYER_PED_ID()))
	{
		if (!PED::IS_PED_INJURED(uLocal_1))
		{
			if (ENTITY::IS_ENTITY_AT_ENTITY(PLAYER::PLAYER_PED_ID(), uLocal_1, vVar0, 0, 1, 0))
			{
				return 1;
			}
...
void func_4()
{
	if (ENTITY::IS_ENTITY_DEAD(uLocal_1, 0))
	{
		uLocal_1 = PED::CREATE_PED(5, iLocal_7, vLocal_2.x, vLocal_2.y, vLocal_2.z, uLocal_5, 1, 1);
