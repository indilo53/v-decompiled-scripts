...
int func_28(int iParam0)
{
	...
	if (iParam0 == 2)
	{
		...
		{
			if (GAMEPLAY::IS_BIT_SET(Global_106565.f_10043.f_108, func_33(func_34(iParam0), iVar1)))
			{
				iVar0++;
			}
			iVar1++;
...
void func_55()
{
	...
	switch (iLocal_411)
	{
		case 0:
			...
			else if (func_50(uLocal_409) && func_50(PLAYER::PLAYER_PED_ID()))
			{
				...
				if (func_60(PLAYER::PLAYER_PED_ID(), uLocal_409, 0) > fVar0)
				{
					func_59();
					if (iLocal_386 == 2)
					{
		...
		case 1:
			...
			if (!func_62())
			{
				if (func_50(uLocal_409))
				{
					if (!UI::DOES_BLIP_EXIST(uLocal_410))
					{
		...
		if (fVar6 > 200f)
		{
			...
			if (func_63())
			{
				...
				if (iLocal_388 != iLocal_391)
				{
					...
					{
						UI::REMOVE_BLIP(&uLocal_390);
					}
				}
				if (!UI::DOES_BLIP_EXIST(uLocal_390))
				{
					uLocal_390 = UI::ADD_BLIP_FOR_RADIUS(vVar3, 200f);
					UI::SET_BLIP_ALPHA(uLocal_390, 128);
					UI::SET_BLIP_COLOUR(uLocal_390, 2);
					UI::_0x54318C915D27E4CE(uLocal_390, 0);
					UI::SET_BLIP_NAME_FROM_TEXT_FILE(uLocal_390, "B_WRE");
					UI::_0x75A16C3DA34F1245(uLocal_390, 0);
					iLocal_391 = iLocal_388;
				}
			}
			else if (UI::DOES_BLIP_EXIST(uLocal_390))
			{
				UI::REMOVE_BLIP(&uLocal_390);
			}
		}
		else
		{
			...
			if (Local_42.f_1.f_4 < Local_42.f_1.f_3)
			{
				if (iLocal_406)
				{
					...
					if (func_63())
					{
						AUDIO::PLAY_SOUND_FROM_COORD(uLocal_408, "SINGLE_BLIP_FROM_BOAT", vVar3, "ABIGAIL_SONAR_SOUNDSET", 0, 0, 0);
					}
					else
					{
						AUDIO::PLAY_SOUND_FROM_COORD(uLocal_408, "SCRIPT_TRIGGERED_FROM_PROP", vVar0, "ABIGAIL_SONAR_SOUNDSET", 0, 0, 0);
					}
				}
				else
				{
					...
					if (!UI::DOES_BLIP_EXIST(uLocal_392))
					{
						uLocal_392 = UI::ADD_BLIP_FOR_RADIUS(vVar3, fLocal_403);
						UI::SET_BLIP_ALPHA(uLocal_392, 255);
						UI::SET_BLIP_COLOUR(uLocal_392, 4);
						UI::SET_BLIP_AS_SHORT_RANGE(uLocal_392, 1);
						UI::_0x25615540D894B814(uLocal_392, 1);
						UI::_0x75A16C3DA34F1245(uLocal_392, 0);
					}
					else
					{
						UI::SET_BLIP_SCALE(uLocal_392, fLocal_403);
					}
					iVar7 = 0;
					if (func_63())
					{
